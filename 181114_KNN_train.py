...
def KNN_recongnition(batch, index, train_quantity = 10000 , k = 30 ):
	...
	if train_quantity <= 10000:
		dist_list = pop_new_data(batch1, dist_list, unknown_data, train_quantity)	
	elif train_quantity <= 20000:
		dist_list = pop_new_data(batch1, dist_list, unknown_data, 10000)
		dist_list = pop_new_data(batch2, dist_list, unknown_data, train_quantity % 10000)	
	elif train_quantity <= 30000:	
		dist_list = pop_new_data(batch1, dist_list, unknown_data, 10000)	
		dist_list = pop_new_data(batch2, dist_list, unknown_data, 10000)
		dist_list = pop_new_data(batch3, dist_list, unknown_data, train_quantity % 10000)
	elif train_quantity <= 40000:	
		dist_list = pop_new_data(batch1, dist_list, unknown_data, 10000)	
		dist_list = pop_new_data(batch2, dist_list, unknown_data, 10000)
		dist_list = pop_new_data(batch3, dist_list, unknown_data, 10000)
		dist_list = pop_new_data(batch4, dist_list, unknown_data, train_quantity % 10000)
	elif train_quantity <= 50000:	
		dist_list = pop_new_data(batch1, dist_list, unknown_data, 10000)	
		dist_list = pop_new_data(batch2, dist_list, unknown_data, 10000)	
		dist_list = pop_new_data(batch3, dist_list, unknown_data, 10000)
		dist_list = pop_new_data(batch4, dist_list, unknown_data, 10000)
		dist_list = pop_new_data(batch5, dist_list, unknown_data, train_quantity % 10000)	
	else:
...
def pop_new_data(batch, dist_list, unknown_data, train_quantity):
	for ii in range(0, train_quantity):
...
for kk in range(5,25):				# kk represent the k of KNN Algorithm
	cnt_true = 0;					# cnt_true represent the present true count pre epoch  
	# Randomly extract data in batch_test 
	for ii in [random.randint(0,10000 - 1) for _ in range(test_num)]:
	...
	# log_file.write('train:', train_num,' test:', train_num,' k:', kk, ' Acc:', cnt_true/float(test_num))

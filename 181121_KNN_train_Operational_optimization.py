...
filename	:   	KNN_train_Operational_optimization.py
...
18/11/21  HYP    3.1          Operational optimization
--------------------------------------------------------------
                                    0ooo
--------------------------ooo0-----(    )---------------------
                         (    )     )  /
...
vis.line(X = torch.Tensor([0]), Y = torch.Tensor([0]), \
										win = 'Accuracy(%)', opts = {'title': 'Accuracy rate(%)'})
...
del cifar10_1, cifar10_2, cifar10_3, cifar10_4, cifar10_5, cifar10_t

nn = NearestNeighbor()					# create a Nearest Neighbor classifier class
nn.train(Xtr_rows, Ytr)					# train the classifier on the training images and labels
for kk in [5, 6, 7, 8, 9]:
	time_start = time.time()
	Yte_predict = nn.predict(Xte_rows, kNN_k = kk, L2 = l2_enable) 		# predict labels on the test images
	# and now print the classification accuracy, which is the average number
	# of examples that are correctly predicted (i.e. label matches)
	time_end = time.time()
	acc = np.mean(Yte_predict == Yte)
	print( 'acc: %f kk = %d test_num = %d time_sec = %d %d' \
									%( acc, kk, test_num, time_end - time_start, l2_enable))
	log_file.write( 'acc: %f kk = %d test_num = %d time_sec = %d %d\n' \
									%( acc, kk, test_num, time_end - time_start, l2_enable))
	vis.line(X = torch.Tensor([kk]), Y = torch.Tensor([acc * 100]), \
					win = 'Accuracy(%)', update = 'append', opts = {'title': 'Accuracy rate(%)'})
log_file.close()
